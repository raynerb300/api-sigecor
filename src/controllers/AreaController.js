...
import { getAreas, getArea, createArea, updateArea, getAreaTypeCorrespondences, getAreaPositions } from '../services/AreaServices.js';
...
AreaController.patch(URL.updateArea, AreaValidator, async (req, res) => {
    updateArea(req.params.id, req.body)
    .then((result) => {
        const response = responseMessage(STATUS('success'), result, MESSAGE('success'));
        res.status(STATUS('success')).json(response);
    })
...
AreaController.get(URL.getAreaTypeCorrespondences, async (req, res) => {
    getAreaTypeCorrespondences(req.params.id)
    .then((result) => {
        const response = responseMessage(STATUS('success'), result, MESSAGE('success'));
        res.status(STATUS('success')).json(response);
    })
...
AreaController.get(URL.getAreaPositions, async (req, res) => {
    getAreaPositions(req.params.id)
    .then((result) => {
        const response = responseMessage(STATUS('success'), result, MESSAGE('success'));
        res.status(STATUS('success')).json(response);
    })
